...
> **时效性说明（2026-08-27 标注）**：本文写于 2026-06-26，描述的是当时的四拍流水线（Think → **Grill** → Do → Verify）。此后流水线演进为 **Think → Do → FuseReview → Verify**：Grill（实施前设计拷问）因流程过重被移除，替换为 FuseReview（实施后独立上下文冷读评审 + 人工检查点）。最新版文章见 [blog-specfuse-workflow.md](./blog-specfuse-workflow.md)。本文作为历史记录保留。
