...
>
> **人：** 全部操作含系统内部，需要 before/after diff。
...
**这是整个流程里 ROI 最高的一刻。** 去掉 diff 后，中间件从"需要理解每个业务模型"简化为"纯 request/response 录像机"。设计复杂度下降一个数量级。如果在 apply 阶段写了一半才发现这个问题，返工成本是现在的 5-10 倍。

其他追问也产出了具体决策：排除 GET 请求减少 90% 日志量、logrotate 10×1GB、graceful shutdown 时 best-effort flush 5s 超时等。Review 后同步更新了 design 和 tasks（17 → 21 项任务）。
